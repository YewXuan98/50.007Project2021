...
def generate_transition_quad_matrix(tag_sequences):
    """Generates a transition triplet matrix"""
    tag_triplets = chain.from_iterable(zip(
                                           chain(["START", "START", "START"], tag_sequence),
                                           chain(["START", "START"], tag_sequence),
                                           chain(["START"], tag_sequence),
                                           chain(tag_sequence, ["STOP"]))
                                       for tag_sequence in tag_sequences)

    transition_matrix = defaultdict(lambda: defaultdict(lambda: defaultdict(lambda: defaultdict(lambda: 0))))
    for first, second, third, fourth in tag_triplets:
        transition_matrix[first][second][third][fourth] += 1
    for v in transition_matrix.values():
        for v2 in v.values():
            normalise_pair_counts(v2)
...
def find_best(
        word: Union[str, None],
        new_tag: str,
        previous_scores: Dict[str, Dict[str, Dict[str, Tuple[float, List[str]]]]],
        e_matrix: Dict[str, Dict[str, float]],
        t_matrix: Dict[str, Dict[str, Dict[str, Dict[str, float]]]],
) -> Dict[str, Dict[str, Tuple[float, List[str]]]]:
    # returns dictionary from prev state to prev prev state to new best score
    output = {}
    for old_tag, old_tag_scores in previous_scores.items():
        output[old_tag] = {}
        for old_old_tag, old_old_tag_scores in old_tag_scores.items():
            max_score = float("-inf")
            max_seq = None
            for old_old_old_tag, (prev_best, prev_seq) in old_old_tag_scores.items():
                if word is None:
                    e_score = 1
                else:
                    e_score = e_matrix[new_tag][word]
                new_score = prev_best + safe_log(t_matrix[old_old_old_tag][old_old_tag][old_tag][new_tag]) + safe_log(e_score)
                if max_score <= new_score:
                    max_score = new_score
                    max_seq = prev_seq + [new_tag]
            output[old_tag][old_old_tag] = (max_score, max_seq)
    return output
...
def viterbi(
    ...
    best_seqs = chain.from_iterable(x.values()
                                    for x in find_best(None, "STOP", best_score[-1], e_matrix, t_matrix).values())
    max_score = float("-inf")
    max_seq = None
...
if __name__ == '__main__':
    for language in lang:
        ...
        t_matrix = generate_transition_quad_matrix(tags)
        ...
        save_prediction(test_words, predictions, language, "4b", filein)
